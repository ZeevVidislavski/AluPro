...
describe('SupplierOrderService', () => {
  ...
  beforeEach(async () => {
    ...
    const serviceModule = await import('../supplierOrderService');
    SupplierOrderService = serviceModule.SupplierOrderService;
  });

  describe('list', () => {
    it('returns data on success', async () => {
      supabase.__setResult({ data: [{ id: 'so1', order_amount: 5000 }], error: null });
      const result = await SupplierOrderService.list();
      expect(result).toEqual([{ id: 'so1', order_amount: 5000 }]);
    });

    it('throws when the query returns an error', async () => {
      supabase.__setResult({ data: null, error: new Error('boom') });
      await expect(SupplierOrderService.list()).rejects.toThrow('boom');
    });
  });
  ...
  describe('create', () => {
    it('injects tenant_id and created_by from the active membership', async () => {
      ...
      await SupplierOrderService.create({ project_id: 'proj-1', supplier_name: 'Acme', order_amount: 5000, order_type: 'aluminum' });
  ...
  describe('update', () => {
    it('strips tenant_id, created_by, and deleted_at from the payload', async () => {
      supabase.__setResult({ data: { id: 'so1', paid_amount: 1000 }, error: null });

      await SupplierOrderService.update('so1', {
        paid_amount: 1000,
      ...
      expect(updatePayload).toEqual({ paid_amount: 1000 });
    });
  });

  describe('delete', () => {
    it('calls the soft_delete_supplier_order RPC, not a direct table update', async () => {
      supabase.__setRpcResult({ data: null, error: null });

      await SupplierOrderService.delete('so1');

      const rpcCall = supabase.__getLastRpcCall();
      expect(rpcCall.fnName).toBe('soft_delete_supplier_order');
      expect(rpcCall.args).toEqual({ order_id: 'so1' });
    });

    it('throws when the RPC returns an error', async () => {
      supabase.__setRpcResult({ data: null, error: new Error('permission denied') });
      await expect(SupplierOrderService.delete('so1')).rejects.toThrow('permission denied');
    });
  });
});
